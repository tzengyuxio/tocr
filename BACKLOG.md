...
- [ ] **`Issue.publishDate` 該不該是必填** — 現在是必填，`publishSort`（由它推導）NOT NULL，而且是 `/magazines/[id]`、`/tags`、`/games`、`/search`、後台期數列表與匯出的預設排序鍵。代價是**沒有日期的期數整本進不了站**（華泰 24 期），或被迫寫一個近似值（遊戲設計大師 21 期）。

  改成可空的話，上面每一處 `orderBy: { publishSort }` 都要決定 null 排哪裡，而每本刊本來就有 `Issue.order` 可以當第二把尺。值不值得，等下一本「有期數、沒日期」的刊出現時再評估（2026-08-22）
