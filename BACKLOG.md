...
- [ ] **一期帶多組編號，`issueNumber` 一個欄位塞不下** — 電玩通同一期的封面與版權頁上有四組並存的編號：
  ...
  **要做的是**：`issueNumber` 存總號，其餘進新的 `Issue.altNumbers String[]`（比照 `Magazine.aliases`）。選 `String[]` 而不是三個具名欄位，是因為**編號數量本來就浮動**——港版卷號要到有港版發行才出現，日後冒出第五種也不用改 schema。代價是沒法單獨查詢或排序港版卷號。

  **網址那半已經做完**（2026-08-17）：slug 用日期發行號、合併號只取第一個日期，規則寫在 [docs/data-conventions.md](docs/data-conventions.md#一期有多組編號時)。剩下的就是這個欄位——電玩通目前 0 期，所以不急，但**匯入電玩通之前必須先加**，否則那三組編號無處可放。
...
- [ ] **給單期一個全域唯一的短碼，當作不會變的握把** — `Issue.code String @unique @default(nanoid(8))` 加一條 `/i/[code]` 轉到正規網址，一個欄位、一行 migration，其餘不動。
  ...
  **判準是「穩定」，不是「短」。** 想單純把現有 cuid 截短是死路：Prisma 的 `cuid()` 是 cuid v1，熵全在尾端（`c` + 8 碼毫秒時間戳 + counter + 指紋 + 8 碼亂數），前綴截多短都在賭運氣。實測 dev 的 764 期，**前 7 碼只剩 12 個相異值**，最大一組 227 期共用；前 9 碼才全唯一，而那也只是碰巧——下次匯入就可能撞，等於已經分享出去的網址會壞。同理，`@@unique([magazineId, code])` 這種「單刊內唯一」的短碼只買到「短」，不值一個欄位。
  ...
  全域碼買到的是別的東西：路徑上的**每一段都會變**——期刊會改名、期號會重排、一期可能同時屬於兩本刊（見下面兩條）。全域碼是唯一不動的那個握把，也正好是「加分享按鈕」需要的東西，以及跟 nostalibrary／cdosgame 對接時的外部識別碼。
