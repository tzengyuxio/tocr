...
- [ ] **其餘改名刊的沿革資料** — 刊名時期功能已上線（電玩通 PlayStation 系是第一條），還有三條刊系待建：**電腦遊戲世界**（158 期都在，後台建 2 筆即可：第 85 期起本名、第 163 期起「遊戲世界」）、**電擊王**（22 期都在，改名後的 10 期（slug `dg-*`）起算「DengekiGAMES」時期，切點看資料）；**電視遊樂雜誌**（→GAME fans）與**電視遊樂報導**（→Super Gamer）要等期匯入後才建得了（起始期是選既有的期）。時期刊頭圖另找（2026-08-22）。
...
- [ ] **電視遊樂雜誌與電視遊樂報導的期數核對基準** — 這兩本站上都是 0 期，將來匯入時拿這組數字
  對（2026-08-24）。
