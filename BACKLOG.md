...
- [ ] **遊戲／標籤頁的文章列表：手機版還沒有封面** — 桌機那半已完成：刊期一欄用
  `IssueCoverHover`，滑過去浮出封面，缺圖有 `CoverPlaceholder` 接著，程式在
  `src/components/ArticleListTable.tsx`。**手機版沒有**——同一支檔案下半的 `md:hidden`
  卡片列表只有標題、副標、刊期、日期、頁碼，全是文字，而「找到那本雜誌」正是這一頁的用途。
  hover 在觸控裝置上不成立，所以手機要的是另一種答案（縮圖排在左邊？點開才展開？），
  不是把桌機那套搬過去（2026-08-14，2026-09-20 縮小範圍）
...
- [ ] [#39] **兩個檔案超過 650 行** — `src/app/(admin)/admin/games/page.tsx` 848、`src/components/article/ArticleForm.tsx` 663（`src/app/(public)/search/page.tsx` 615 在門檻邊上）。不急，也不建議為了拆而拆；列著是為了下次動到它們時順手處理，不是排一個專門的重構。
  ...
  **2026-09-20 覆核：條件到了，而且檔案還在長。**`games/page.tsx` 從 848 漲到 **931 行**
  ...
  動內部的改動」已經排在前面了：〈遊戲索引頁的檢索要整體重想〉要加平台等篩選、重排控制項
  版面，勢必動到這支的查詢與版面兩端。**兩條一起做**，不要先拆再改或先改再拆。
...
- [ ] **目錄頁圖片點開後要能邊看圖邊對條目，不是單純的 lightbox** — 現在單期頁點開 `tocImages` 只是放大看圖。改成左右兩欄：左邊目錄頁圖片、可切換上下張，右邊該期的目錄條目、可上下捲動，這樣查對時不必在兩個畫面之間跳。錄入與校對都會用到（2026-09-20）

- [ ] **遊戲索引卡片的留白來自 `Card` 自己的垂直 padding** — 2026-09-20 的 #163 把縮圖從 56×56 改成 72×96 的 3:4 之後，卡片看起來空得很明顯。量過（1440px）：`Card` 303×162、`CardContent` 301×120（`p-3`）、圖 72×96、文字區只有兩行共 44。算式是 `卡高 = max(圖 96, 文字 44) + CardContent 24 + Card 42`——**那 42px 是 `Card` 的垂直 padding，換圖前就在，只是圖小時看不出來**。給這張 `Card` 一個 `py-0` 高度就回到 120，圖上下貼齊，不必動圖也不必改欄數。（別去找「圖的 margin」，圖沒有 margin。）頁面在 `src/app/(public)/games/page.tsx`（2026-09-20）

- [ ] **表格的排序改成點表頭，不要在上方另擺排序按鈕** — 點表頭比較直覺，也讓全站表格的操作一致。現在 `/games`、文章列表那幾張各自在上方放排序控制項（`src/lib/game-browse.ts`、`src/lib/article-listing.ts` 是排序邏輯的落點）。要一起想的是：排序狀態仍然放網址（現在就是），以及手機寬度下表頭點得到嗎（2026-09-20）

- [ ] **首頁的統計數字移到不顯眼的位置** — 雜誌／期／文章／遊戲／標籤的總數，加上要補的封面數與目錄數，擺在首頁主要位置太搶戲。搬到 footer，或另找更合適的地方。頁面在 `src/app/(public)/page.tsx`（2026-09-20）


- [ ] **10 組遊戲的維基連結指向同一個單作條目，其中一邊指錯** — 2026-09-20 清完廠商條目（44 條）與清單條目（3 條）之後剩下的一類：兩款遊戲共用同一條維基連結，而那是個**單作**條目不是系列條目。確定錯的兩組：`夏季運動會` 指向 **Winter** Challenge（另一款是 `雪山競技`）、`3 x 3 EYES 吸精公主` 與 `轉輪王幻夢` 都指向 `三只眼`（那是**漫畫**條目不是遊戲）。其餘八組是續作指向一代的條目（`殖民計劃2`→`殖民計劃`、`戀愛物語 2`→`エーベルージュ`、`少女魔法師2`、`守護者之劍外傳`、`天使戀曲`、`天使協奏曲`、`新幻想空間`、`明星撲克`／`撲克萬花筒`→`Hoyle's Official Book of Games`）——續作沒有獨立條目時指一代算退路不算錯，但讀者點過去會以為那就是它，要逐筆判。**指向系列條目的 33 條不在此列**，那是 yuxio 決定保留的合理退路。查法：`site='WIKIPEDIA'` 依網址 group by，取出現兩次以上的（2026-09-20）
