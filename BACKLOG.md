...
- [ ] **《電玩通》早期四期的目錄頁碼有整群錯位** — 2026-09-13 用封底裡的遊戲索引替那批 41 期補掛遊戲時撞到的：同一期裡好幾篇文章共用同一個 `pageStart`，而索引在那一頁印的是別的遊戲，所以錯的是目錄的頁碼不是索引。**成群的在最早辨識的四期**：VOL.181 有 10 篇同掛 p100、VOL.193 有 8 篇同掛 p101、VOL.184 有 6 篇同掛 p109、VOL.185 有 6 篇同掛 p112（另 VOL.284 有 4 篇同掛 p108）。這是 memory `famitsu-toc-ocr-pitfalls` 記的「COLUMN 清單整欄錯位一列」的實例，要回頭對目錄圖逐篇改頁碼。其餘 21 期各有 2 篇共用一頁，那**可能是真的同頁**（欄目與遊戲同頁起），要看圖才知道，別一律當錯。**頁碼修好後不必手工掛關聯**——`link_famitsu.py`（頁碼嚴格相等才接）再跑一次就會把這 66 篇補上（2026-09-13）
...
- [ ] **《電玩通》11 期的封面人物要補回姓名中間的空格** — 寫法 2026-09-13 定案為**照封面印的**，
  而《電玩通》封面對日本藝人一律分寫（`水樹 奈奈`、`石田 亞佑美`），目錄頁的 COVER STAFF 欄
  才是不分寫的。當天照 COVER STAFF 填的 11 期（316、317、326、329、333、338、343、363、412、
  416、420）要改成封面寫法；非日本姓名那 6 期（324、330、336、361、381、385）兩處一致，不動。
  上游那 43 期帶空格的與封面相符，**不是待辦**。改法見 `docs/data-conventions.md` 的「封面資訊」
  （2026-09-06 提出，2026-09-13 定案，當晚查明方向相反）
