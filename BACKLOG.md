...
- [ ] **單期的網址還是 cuid** — `/magazines/ace/issues/cmsu8sugt000004jvzp8pyvks`，看得出是哪一本、看不出是哪一期。
  ...
  **遊戲與標籤**（2026-08-16）用中文 slug、**期刊**（2026-08-17）用 ASCII slug，都已上線，舊 cuid 308 轉址，見 [docs/features.md](docs/features.md) 的「公開網址」。**動機要更正**——當初寫的是「太長」，但實測中文 slug percent-encode 之後比 cuid 還長（36–54 vs 25）。換到的是**可讀可猜**，不是長度。**文章不需要**，沒有公開的單篇網址。
  ...
  - **以日期為號**：電玩通有連續總號，也有 `2004年7月16日號`；還有 `電玩通 No.225 2009年1月9・16日合併號（期別 2008 47）` 這種三種編號並存的
  - **專名**：創刊號、試刊號、試刊2號、增刊號、特輯、特刊、創刊驚嘆號、夏季熱鬧增刊
  - **合併號**：`70+71`

  可行的方向是「人工為主、自動為輔」：加 `Issue.slug` 與 `@@unique([magazineId, slug])`，只在純數字時給預設值，其餘留給人填，撞到就報錯要人取名（不自動接 `-2`），再寫一組編目慣例進 `docs/data-conventions.md`。**但先把下面三條資料模型的問題想清楚**，否則 slug 只是把問題搬到網址上（2026-08-17 更新）
...
- [ ] **電玩通台版與港版各自編號，該是同一本刊嗎** — 香港比台灣晚幾個月發行，有獨立的期號序列。若當成同一本 `Magazine`，期號會有兩套互相衝突的序列；若分成兩本，`fmt-tw` 之外要再給港版一個 slug，而兩邊共用刊名與 ISSN。

  先決定這件事，單期的網址與期號慣例才有意義（2026-08-17）
