...
- [ ] **《勝利土星少年》整本沒收** — 《土星少年》第 47 期起與《勝利少年》合併成的刊，兩本的 `description` 都記了這件事。合併後的期號怎麼接續、是不是該當成其中一本的刊名時期，都要看到實物才判得出來（2026-09-21，收藏者轉述）
...
- [ ] **後台複查的全螢幕對照還沒有人看過版面** — PR #195 已在正式站：複查畫面右下膠囊多一顆「全螢幕對照」，按下去掃描與文章列表一起撐滿畫面（Esc 離開，正在改某一列時 Esc 屬於那一列）。型別／lint／測試全綠，但 `/admin` 要登入，我點不到，**版面沒有任何人看過**。要看的兩處：掃描那欄現在是固定高度的深色欄（短掃描會有上下黑邊，原本是貼著圖高的淺色框）、全螢幕時 62/38 的分欄比例。不順手的話兩處都是改一行（2026-09-21）
