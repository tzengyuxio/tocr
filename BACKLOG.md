...
- [ ] **雜誌、單期、文章的網址還是 cuid** — `/magazines/cmsp9xg7l000q04l1nug2zzrz/issues/cmsu8sugt000004jvzp8pyvks` 有 66 個字元是 ID，看不出是哪一本、哪一期。

  **遊戲與標籤已經做完**（2026-08-16，見 [docs/features.md](docs/features.md) 的「公開網址」）：改吃中文 slug，舊 cuid 308 轉址。**動機要更正**——當初寫的是「太長」，但實測中文 slug percent-encode 之後比 cuid 還長（36–54 vs 25）。換到的是**可讀可猜**，不是長度。

  剩下這三個難在資料面，不是難在路由：

  - **`Magazine` 沒有 `slug` 欄位**，要新增並回填 30 本（`data/magazines.json` 裡的 `cgw`、`swm`、`ace` 是匯入用的資料檔，不是資料庫欄位——原本這條寫錯了）
  - **`Issue.issueNumber` 含中文**（`創刊號`、`VOL.51`、`第163期`、`70+71`），要另存 URL 安全的 slug，或退而用 `order`。已有 `@@unique([magazineId, issueNumber])` 可倚靠
  - **`Article` 數量會上看數十萬**，slug 撞名的機率跟前兩者不是同一個級距，可能該維持 id 或用短亂數
