...
- [ ] **43 期帶空格的封面人物還沒收斂** — 寫法已於 2026-09-13 定案為**照封面印的寫**（《電玩通》的 COVER STAFF 欄印中文譯名、姓名不空格），當天照此填了 17 期、順手改掉 4 期上游留下的空格版。**剩 43 期仍是空格版**：《電玩通》28、《電擊王》15，全是上游匯入的，封面不在手上，要逐期翻圖才改得了。查法：`cover_subjects` 裡比對 `[一-鿿]\s+[一-鿿]`。判讀時注意**低解析度會讀出不存在的空格**，要裁到原始解析度看（2026-09-06 提出，2026-09-13 定案）
...
- [ ] **《電玩通》VOL.335、336 的遊戲索引沒掃到** — 這兩期的索引不在封底裡，目錄頁標的是 P.120 與 P.118，而內頁沒掃。`data/famitsu-game-index.csv` 因此是 42 期而不是 44 期。要補得回頭翻這兩本的那兩頁。掃描前先看目錄頁右下角那一格寫「封底裡」還是頁碼（2026-09-13）
