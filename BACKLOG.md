...
- [ ] **後台頁面切換明顯卡頓** — 本機量測後排除了查詢與渲染：production build 各頁 server render 2–14ms，dev 暖機後 60–90ms（#18）。已加上 `PERF_LOG=1` 的伺服器端量測，待在實際部署收數據；也要一併確認 dev 的 on-demand 編譯與 Neon scale-to-zero 冷啟的影響（2026-08-12）
