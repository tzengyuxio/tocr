...
- [ ] [#39] **兩個檔案超過 650 行** — `src/app/(admin)/admin/games/page.tsx` 848、`src/components/article/ArticleForm.tsx` 663（`src/app/(public)/search/page.tsx` 615 在門檻邊上）。不急，也不建議為了拆而拆；列著是為了下次動到它們時順手處理，不是排一個專門的重構。
  ...
  **2026-09-20 覆核：條件到了，而且檔案還在長。**`games/page.tsx` 從 848 漲到 **931 行**
  ...
  動內部的改動」仍然沒來。**2026-09-20 二次更正：先前寫「遊戲索引檢索重想會動到這支，
  兩條一起做」是搞錯了檔案**——檢索重想動的是 `(public)/games/page.tsx`，這一條講的是
  `(admin)/admin/games/page.tsx`，兩支唯一的交集是後台從 `game-browse.ts` 匯入
  `GAME_SORTS` 與 `GameDirection` 兩個名目，而那正好是加篩選不會動到的部分。
  檢索重想已於同日做完（PR #183），這一條沒有跟著被推進。
...
- [ ] **文章表格的排序改成點表頭，不要在上方另擺排序按鈕** — 點表頭比較直覺，也讓表格的操作一致。落點是 `/games/<遊戲>`、`/tags/<標籤>` 那幾張真的用 `<Table>` 的文章列表（`src/lib/article-listing.ts` 是排序邏輯）。要一起想的是：排序狀態仍然放網址（現在就是），以及手機寬度下表頭點得到嗎。**`/games` 索引頁不在此列**（2026-09-20 與 yuxio 確認）——它的列表不是 `<table>` 而是 flex 排的列，沿用 `MagazineList` 的先例（`<Table>` 是 `"use client"`，為了畫幾條線把 50 列送去 hydrate 不划算，手機上也擠不下），沒有表頭可點，排序留在工具列（2026-09-20）
