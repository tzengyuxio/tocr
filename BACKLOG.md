...
- [ ] **封面人物的日本藝人怎麼寫還沒定** — `coverSubjects` 是純字串陣列，寫法分岔就歸不到一起。要定的三件：記日文原名還是中文譯名（`友坂理恵`／`友坂理惠`，漢字字形本身就兩套）、姓名之間空不空格、片假名藝名怎麼處理。《電玩通》那批進來之前要先有答案，屆時已錄入的也要一起收斂。藝名前綴不記（`GiGi 梁詠琪` → `梁詠琪`）已定案並寫進 [docs/data-conventions.md](docs/data-conventions.md) 的「封面資訊」（2026-09-06）
