...
從當時的 172 對「建議合併」隨機抽 20 對逐一核對，**命中 19/20 = 95%**，錯的那一對
就是上面的《方程式機車賽》／《方程式賽車》。

**但 20 個樣本撐不起「整批信」**：Wilson 95% 信賴區間是 76%–99%，外推到 172 對
是預期錯 9 對、區間 2–41 對。20 個樣本分不出「95% 準」與「76% 準」，而合併不可逆。
要把下界收到 90% 大概得驗到 60–80 對。在那之前，**不要把「建議合併」整批當成已判**
——照 `triage` 排序從高分往下人工按，後台的合併對話框每次都會先 dry-run。
