...
- [x] **目錄頁掃描點開後改成左圖右目錄**（2026-09-20） — 單期頁點 `tocImages` 原本開的是
  ...
  **跟圖有關的控制項貼在圖上**，不在標題列——它們改的是左欄看到的東西，手要伸過大半個
  螢幕去按不合理（2026-09-20 yuxio 指出）。膠囊裡是翻頁、雙頁並列、原尺寸三組：跨頁印的
  目錄兩張本來就是連著讀的；而整頁塞進半個視窗的掃描字只剩一半大，看得出哪一列讀不清是
  什麼字，所以原尺寸那一邊左欄自己捲。兩者都預設關著。
