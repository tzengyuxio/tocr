...
- [x] **書稿搬到自己的 repo**（2026-09-23） — `docs/book/` 的 72 個檔與 `docs/backlog/book.md`
  搬到私有的 `gamepress`（Forgejo，hikari）。**tocr 是公開 repo，書稿放在這裡等於把撰寫計畫
  一起公開**——原本只有 `_scripts/work/` 被 gitignore 擋著，稿本身從 PR #149 就公開了。

  搬的是 `docs/book-snapshot-0914` 分支的版本不是 main：那條分支有 5 個沒進 main 的 commit，
  雜誌從 44 本長到 56 本，才是書稿真正的狀態。分支搬完即刪。

  新 repo 一次收四本遊戲書（圖鑑、中文 DOS 遊戲寶典、這款遊戲你沒玩過、影響我的 100 款遊戲），
  因為**它們吃同一批站台資料**，fetch/gen 管線只要寫一份。`_scripts/` 改名 `_tools/` 並參數化：
  `BOOK_DIR` 從「fallback 到 tocr 的絕對路徑」改成必填，`fetch.sh` 的站台抽成 `API_BASE`。

  **歷史沒有改寫**——`git rm` 只讓它從 HEAD 消失，舊 commit 裡還在。這是刻意的：內容已經公開
  一陣子，改寫歷史換來的隱蔽性不存在，代價卻是所有 hash 變動。目的是「之後新寫的不公開」，
  `git rm` 就達成了。


- [x] **後台複查也用同一套對照版面**（2026-09-20） — 複查畫面本來就是左掃描右列表，缺的是
  「把它撐滿畫面」：原本那顆「全螢幕檢視」開的是單張放大的燈箱，會把正在對的列表整個蓋掉，
  複查的人要嘛記著、要嘛關掉再開。改成把掃描與列表這一整塊撐滿畫面，列表還在右邊、還能編。

  **切換只改外框的 class**，列表那棵樹不重掛——不然正在改的那一列會被切掉，而那是 dnd-kit
  的 SortableContext 加上每一列自己的草稿狀態，重掛的代價很實在。

  看掃描的那一半抽成 `TocScanPane`，公開單期頁的對照視窗與後台複查共用：兩邊做的是同一件
  事，只是一邊在讀、一邊在改，翻頁／雙頁／原尺寸／拖著看不該有兩套。順帶把貼齊高度的算法
  從「用視窗高度扣一堆」改成「對框算」（`h-full` 的外框 + `max-h-full` 的圖），因為同一個
  元件現在會被放進三種大小不同的框裡。

- [x] **目錄頁掃描點開後改成左圖右目錄**（2026-09-20） — 單期頁點 `tocImages` 原本開的是
  燈箱，一張圖蓋住整個畫面；而讀掃描頁的人幾乎都在對照站上的條目（頁碼對不對、標題有沒有
  漏、一欄底下的小標是不是各自成篇），燈箱等於逼人在兩個畫面之間來回記。改成滿版的對照
  視窗：左邊掃描、上下張可切（含左右鍵），右邊該期目錄自己捲。窄螢幕上下疊。

  **所有控制項都貼在圖上，沒有頁首那一列**——它們改的是左欄看到的東西，手要伸過大半個
  螢幕去按不合理（2026-09-20 yuxio 指出，連標題與關閉也一起搬）。下緣的膠囊裡是翻頁、
  雙頁並列、原尺寸；刊名與期號在左上角，關閉在右上角。讓出來的那一列全給了掃描，貼齊
  高度的圖因此高了約 8%。

  **關閉不在膠囊裡**（2026-09-20 yuxio 指出）——那一排是「怎麼看這張圖」，關掉不是其中
  一種看法，而右上角是關窗本來就會去找的地方。點圖以外的地方也關得掉，免得兩顆都沒找到；
  點圖本身則是放大，跟封面燈箱一致。

  **原尺寸時按住圖可以拖著看**（2026-09-20 yuxio 指出）。捲軸還在，但看著一張圖的人第一個
  動作是抓住它拖。只接手滑鼠：觸控本來就捲得動，插進去只會跟原生的慣性捲動打架。

  雙頁並列與原尺寸都預設關著：跨頁印的目錄兩張本來就是連著讀的，但按下去的是某一張
  縮圖，點了第 2 頁卻跳出 2、3 兩頁會嚇到人；而整頁塞進半個視窗的掃描字只剩一半大，
  看得出哪一列讀不清是什麼字，所以原尺寸那一邊左欄自己捲。

  **目錄頁因此離開了燈箱那一串**。封面與其他圖片仍舊是燈箱（它們沒有可對照的東西），
  代價是從封面按右鍵翻不到掃描頁了——同一張圖有兩種點法比較糟。

  **右欄是頁面上那一份目錄本身**（`IssueTocList`，server component 算好當 prop 傳進
  client 的對照視窗），不是另外畫一張簡表：對照用的清單跟頁面上的長得不一樣就沒有對照的
  意義。代價是 RSC payload 裡那段出現兩次，視窗關著時它不在 DOM 裡。

- [x] **首頁的統計數字搬到頁底**（2026-09-20） — 五格 `StatGrid` 加一行涵蓋數原本擺在 hero，
  會先於搜尋框被看見，而搜尋框才是那一頁的功能。改成頁底一行小字，接在「最新單期」後面。

  **沒有搬進全站 footer**（條目原本寫的其中一個去處）：那要讓 `(public)/layout.tsx` 變成
  會查資料的 async component，首頁的 ISR（`revalidate = 60`）與其他靜態頁都會被迫轉成
  動態算。搬到首頁底部沒有這個代價——那幾個數字本來就已經在這一頁查了。

  **一行不是五格**：五個總數之間沒有主次，做成卡片會逼讀者一格一格讀完，連成一句話反而快。
  有自己頁面的（雜誌、遊戲、標籤）連過去，單期與文章沒有總表所以不連。`StatGrid` 元件留著，
  後台兩頁還在用。


- [x] **遊戲索引頁的檢索整體重想過了**（2026-09-20，PR #183） — 原本的三個維度（關鍵字、
  「全部／多篇報導」、依名稱或文章數排序）換成關鍵字、**報導年代**、平台。

  **第一個缺口不是平台。**條目原本寫「至少要加平台篩選」，但先量了一輪才發現那個前提
  只對了一半：`Game` 自己的欄位確實補了一些（platforms 1,353），可是**最稠密的軸根本
  不在 `Game` 上**，在關聯的另一端——`articleGames → article → issue.publishSort`
  算得出「這款遊戲是哪幾年的雜誌寫的」，6,735 款有文章的遊戲全都有，涵蓋率 100%。
  平台那 1,353 筆反而是兩叢的：cdosgame 的 DOS 遊戲與《電玩通》2010 年代的主機，中間
  整段是空的（FC 0、SFC 0、MD 0、DC 0、GB 0、PS 1）。直接上平台篩選會讓讀者得出
  「這個站只有 1 款 PlayStation 遊戲」——比沒有篩選更糟，因為錯的分面看起來像答案。
  所以年代是主軸、平台是次要的軸而且畫面寫明覆蓋率。

  **「多篇報導」拿掉了，沒有替代品**（yuxio 的決定）：它當初是為了「找出跨單期、看橫跨
  多本雜誌的呈現」而做的，那時目錄文章還少、符合的遊戲難找；現在 1,914 款有兩篇以上、
  340 款跨雜誌，隨手就碰得到。篇數留給排序。

  **列表成為預設檢視**，卡片留在 `?view=cards`——6,754 款裡有封面的是 45 款（0.7%）。
  版面先畫 mockup 談過才動手。年代長條可以拖著選一段也可以點兩下定起訖，而每根長條
  仍然是 `<a href>`，沒有 JS 也能用。一頁 40 筆改成 50 筆。

  用量佐證：正式站近 60 天 `/games` 203 次瀏覽，`?filter=`／`?sort=`／`?q=` **各 0 次**，
  只有 `?page=2..5` 共 36 次；同期 `/magazines?view=list` 50 次、`?filter=tv` 23 次。
  所以不是這批讀者不用控制項，是 `/games` 原本那兩個維度不對。
...
- [x] **`nameKey()` 把假名與諺文整段丟掉**（2026-09-20，PR #181 與 #184） — `slugify()` 的字元集
  只留 `a-z0-9` 與中日漢字，其餘當標點剝掉——**但假名與諺文是字**。正式站上量到的實害：20 筆
  遊戲完全沒有鍵（「ぷりんせすでんじゃあ」正規化後是空字串），slug 只好 fallback 成
  `game`、`game-2`…`game-20`；另外 9 筆退化成純數字（「カスタムメイト・2」→ `2`、
  「임진록2」也是），撞得到任何帶同一個數字的名字。**它同時擋住原名的錄入**：把
  `三國志リターンズ` 寫進 `nameOriginal`，鍵會變成 `三國志`，與站上 13 篇文章的《三國志》撞號。

  修法是把字元集加上平假名、片假名（`・` U+30FB 留在分隔符，`ー` U+30FC 是字）與諺文。
  **冒號考慮過但不加**：實測留下冒號會把 `三國風雲2：風雲再起`／`三國風雲2-風雲再起` 這組同款
  拆成兩筆，對 cdosgame 的命中也少 65 個——冒號是「這一期印、下一期不印」的標點，折掉它正是
  這把尺要做的事。全形半形不必另外處理，`NFKC` 跑在第一步。

  換規則的風險實測是零：站內撞鍵組數 19 → 19，沒有組被拆開也沒有新碰撞，1,110 個標籤沒有一個
  含假名。原本以為會拆開一批，結果那些碰撞**不是站內兩筆互撞，是站上一筆撞到上游 cdosgame 的
  別名**。`scripts/backfill-name-keys.ts` 回填了 145 筆 `nameKeys`、20 筆 fallback slug，
  `--reslug` 再重算 125 筆被挖空的 slug（`エリア 88` 的 slug 本來是 `88`）。

  **一個代價要記著**：`純愛手札`／`ときめきメモリアル～Forever With You` 這組合併候選消失了。
  它原本成立是因為片假名被剝光後剩 `foreverwithyou`，剛好對上上游別名——配對是對的，成立的
  理由是錯的。要正當地找回來就在站上那筆補原名或別名，而那正是這個修正解鎖的事。

- [x] **OCR 回應帶前導零的頁碼會整份解析失敗**（2026-09-20 查核：早已修好） — `ocr.utils.ts`
  的 `stripLeadingZeroNumbers()` 已經寫好並接進 `parseOcrResponse()` 的 repair 鏈，錨在行首的
  key 上（`^(\s*"[A-Za-z]\w*":\s*)0+(\d)`），所以不會誤改 summary 字串裡的冒號。條目沒跟著收，
  是這次清查才發現的。

- [x] **遊戲頁卡片的圖改成長方形**（2026-09-20，PR #163 與 #173） — 縮圖從 56×56 方框改成
  72×96 的 3:4，缺封面改用 `CoverPlaceholder`（它的判準是「3:4 且寬度 64px 以上」）。
  #173 再收掉 `Card` 自己的 `py-5`，卡片高度從 162 回到 122——那 42px 換圖前就在，
  只是圖小時看不出來。

- [x] **`TableCell` 的 `whitespace-nowrap` 預設翻過來了**（2026-09-20） — shadcn 原本給每一格
  `whitespace-nowrap`，在只裝短值的後台表格看不出問題，但只要有一欄裝自由文字就會撐到
  max-content、整張表超出容器。改成**換行是預設、不換行由欄位自己指定**：短值（日期、頁碼、
  筆數、按鈕列）本來就填不滿一欄，有沒有這個宣告都不會換；需要保證不斷行的 25 處補上
  `whitespace-nowrap`，散在 8 支檔案。這樣讀程式碼時「這一欄不能斷」是個看得見的決定，
  不是繼承來的。`truncate` 自帶 nowrap 的四處沒動；`TableHead` 維持不換行——表頭是短標籤，
  斷行只會讓高低不齊。

- [x] **「從 RAWG 抓取」的實作檢查**（2026-09-20 封存） — 檢查本身 2026-08-22 就做完了：
  四個問題裡「使用者看不到錯誤」已修、「沒有 key 不顯示按鈕」已做、「`remotePatterns` 會擋」
  查證後不成立（前台兩處都帶 `unoptimized`）、「取第一筆」已改成回五筆候選讓人挑。剩下的
  **不是實作問題而是 2026-08-22 的押後決定**——RAWG 給的是宣傳美術／截圖不是盒裝圖，挑對
  遊戲也拿不到對的圖。那個決定的後果各有落點：「三筆 `coverImage` 是 RAWG 的截圖」是它
  具體長出來的髒資料，「把遊戲的 `nameEn` 填起來」是英文查詢這條路的前置。詳情留在
  [rawg-fetch-review.md](rawg-fetch-review.md)。

- [x] **《攻略月刊》已建檔，還缺期數與刊頭**（2026-09-20 封存：已過期） — 這條 2026-08-25
  記的時候站上一期都沒有。現在有 3、4、5、6、9 五期而且都有目錄圖，刊頭也在
  `~/Pictures/covers/derived/logos/sources.csv`（取自 NO.4）。真正還缺的只有 NO.7、NO.8，
  而那已經是獨立的一條；那五期的目錄還沒辨識，歸「有目錄圖卻沒辨識」那一族。

- [x] **遊戲頁的文章表格不再被長標題撐出水平捲軸**（2026-09-20，PR #165） — 根因不是欄寬沒設
  上限，是 shadcn `TableCell` 預設帶 `whitespace-nowrap`。量到 viewport 820px 時表格 1568px、
  容器 694px，撐寬的是《軟體世界》113 期那筆把一串遊戲名寫進副標的條目。只讓標題那一格恢復換行就
  收回容器寬度。**共用元件的預設沒動**，它被 13 個檔案用著，另記成 BACKLOG 一條。

- [x] **單期頁「封面資訊」box 的 credit label 改由值決定**（2026-09-20，PR #165） — 原本一律寫
  「封面」，在已經叫「封面資訊」的 box 裡說了等於沒說；自帶角色詞的值還會讀成「封面：攝影：…」
  兩個冒號。改成把值裡的角色詞提上來當 label，裸名字則標「繪者」（`coverCredit` 的預設語意）。
  欄位與值的寫法都沒動。正式站 96 期有值，其中 36 期自帶角色詞（`設計：` 33、`攝影：…；封面製作：` 1、
  `封面製作：` 1、`提供：SEGA` 1）。

- [x] **首頁顯示收錄的涵蓋數**（2026-09-20，PR #165） — 五個總數底下加一行「其中 N 期有封面、
  M 期已錄入目錄」，都相對於單期總數講。上線值為 1392 期有封面（50%）、343 期已錄入目錄（12%）。
  做成一行小字不占 StatGrid 的格子：它們不是第六、第七個總數，七格在 sm 以上也排不成一列。

- [x] **2026-09-19 匯入的那批目錄已改成按頁碼排序**（2026-09-20） — 17 期全部送過
  `PUT /api/articles/reorder`，14 期實際動了順序（電視遊樂雜誌 140、149 與電視遊樂報導 96
  本來就是頁碼序，端點自己判定沒變就不寫）。排序鍵疊兩層穩定排序：先照原本的 `sortOrder`、
  再照 `pageStart`，所以同一頁的子項目仍保持目錄上的閱讀順序（欄目底下的子條目共用欄目那一頁）。
  **沒有頁碼的 5 篇排在最後**（電視遊樂報導 125 三篇、73 一篇、電擊SS 創刊3號 一篇）——缺頁碼
  不是第 0 頁。驗過：17 期都沒有一筆的 `pageStart` 小於前一筆，那 5 篇的 `sortOrder` 也都大於
  該期最後一筆有頁碼的。

- [x] **《電玩通》早期五期的目錄頁碼錯位已修，66 篇掛不上關聯的收掉 38 篇**（2026-09-13） —
  錯的是目錄頁碼不是索引：OCR 把「新作情報快遞（與期待新作特搜隊）」底下的子項目
  全給了欄目那一頁。用封底裡的索引回填，**29 篇改了 `pageStart`**
  （VOL.181 九篇、184 五篇、185 五篇、193 七篇、284 三篇），接法是標題與索引名逐字相同、
  且索引在該期只指向一個頁碼才改。改完 181／284 的 12 篇補掛了遊戲，184／185／193
  那 17 篇先前人工掛過、內容正確，沒動。

  **剩下的 B3 其實不是錯位**：那一群是「欄目與它第一款遊戲印在同一頁」，頁碼本來就對，
  只是 `link_famitsu.py` 把多篇共用一頁一律當可疑跳過。用同一條嚴規則補掛了 26 篇，
  《電玩通》41 期的關聯從 422 篇升到 **460 篇（47%）**。還沒掛的 29 篇是欄目本身
  （標題是欄目名不是遊戲名），那一頁的遊戲已經掛在同頁的子項目上，不重複掛。
...
- [x] **《電玩通》11 期的封面人物補回姓名中間的空格**（2026-09-13） — 同日稍早照目錄頁的
  ...
  已改回封面寫法（`水樹 奈奈`、`石田 亞佑美`）。非日本姓名那 6 期（324、330、336、361、
  381、385）兩處本來就一致，沒動；上游匯入那 43 期帶空格的與封面相符，本來就不是待辦。

  **這條的起因是拿目錄頁去校正封面**：封面與 COVER STAFF 欄對日本藝人的印法系統性不同，
  兩處本來就不一樣，不能互相校正。規則定案為照封面印的，寫進
  [docs/data-conventions.md](../data-conventions.md)；單期頁的「人物」標籤同時改用全形冒號，
  因為值本身含真實空格，空格不能當分隔符。

- [x] **《電玩通》VOL.335、336 的遊戲索引其實掃到了**（2026-09-13） — 原記「索引不在封底裡、
  內頁沒掃」是判錯：兩期的封底裡（`famitsu-tw_335_c3.jpg`、`_336_c3.jpg`）整片印著
  「電玩通週刊VOL.N遊戲索引」，只是目錄頁右下角標的是 P.120／P.118。已轉錄 79 列進
  `data/famitsu-game-index.csv`（335 共 39 列、336 共 40 列），CSV 從 42 期 1648 列變成
  44 期 1727 列。**教訓是別從目錄頁的標示推封底裡有沒有索引**，看圖才算數（README 已改）。

- [x] **`tvgame-magazine` 第 28–33 期的封面與發行日錯掛一期**（2026-09-05） — yuxio 指出站上
  第 28 期的封面其實是第 29 期的，一查發現整段往前錯掛：站上 28／31／32 掛的分別是
  29／32／33 的封面與出刊日，blob 檔名（`tvgm_no-029.jpg` 等）本來就寫著正確期號，
  是掛錯期不是傳錯檔。三期各往後移一格，第 28、31 期改掛 IA 掃描的真封面
  （`NO 28`／9月5日、`NO. 31`／10月17日），發行日一併改成 1988-09-05 與 1988-10-17。
  兩端驗過沒有錯位（22、23、37 期封面都與站上一致）。

  **掛錯的圖看起來就是「已經有了」**——同日跑的 IA 批次腳本只補站上缺的圖，冪等檢查
  擋不住錯的資料，所以當時沒有覆蓋這兩期。順帶補完第 72 期（1990-06-28、135 元、
  3周年社慶特刊）。細節見 [tvgame-magazine-dates.md](tvgame-magazine-dates.md)
