...
- [x] **《電視遊樂》系列從 archive.org 接回站上**（2026-09-05） — IA 的 `tvgame-magazine`
  16 期掃描全數接上：`tvgame-report` 建《電視遊樂情報》1–18 期（269→287 期）、補 8 期發行日
  （有日期的 16→24 期）、傳 9 張封面與 26 張目錄圖（這兩本先前一張目錄圖都沒有）、
  每期掛一條 IA 深連結。

  **IA 深連結不帶 `.pdf`，而且「回 200」不是驗證**：`/details/` 底下任何路徑都回 200，
  連 `bogus-nonexistent-file` 也是——通通渲染成 item 首頁。要看回傳 HTML 有沒有
  `subPrefix=<檔名>`。原本 16 條全帶 `.pdf`，等於全都只連到首頁。

  同日連帶：期號規則從「照封面存」收斂成「中性的 `No.`／`Vol.` 不進資料、只存數字，
  顯示時補『第 N 期』」，全站改 1424 期；前綴承載語意時才留，寫成全大寫不加空格不補零
  （`情報 NO.1`、`SuperGamer NO.40`、`DengekiGAMES VOL.2`）。規範進 data-conventions.md。
  《電視遊樂報導》的 `description` 補上「初名電視遊樂情報」與「日本 ASCII
  《ファミコン通信》授權中文版」（最早見於第 114 期封面，第 106 期還沒有）、
  建了「情報→報導」兩段刊名沿革。細節見
  [tvgame-report-info-lineage.md](tvgame-report-info-lineage.md)

  **同日再補 37 期發行日**：站上有封面而沒日期的期，日期都印在封面右上角期號下方，
  逐張讀出來寫入（第 40 期–第 103 期，1990-05 至 1993-02），有日期的期數 25 → 62。
  第 87 期封面只印「5月22日發售」，年份由前後兩期夾出來，寫進該期 `notes`；
  1998新春合併號只印 1998 與「新春」，按春節（1 月 28 日）推得 1 月初出刊，EDTF 填到月。
  創刊日改從情報 NO.1 起算（1987-11），`knownIssueCount` 342 → 360 並重寫來源註。

- [x] **新一批掃描的封面接回站上**（2026-09-05） — 期刊掃描檔從 188 張長到 265 張。
  `covers_vs_tocr.py --fill-codes` 對正式站回填 `tocr_issue` 124 列中的 117 列，
  `victory-boy` 創刊號（站上是中文期號，數字對不上）手動補；建了《攻略快報》首期
  `革新7號`（該刊在站上原本 0 期）與《新遊戲時代》兩期特刊（`攻略特刊①`、`正宗攻略寶典3`），
  三張 c1 也傳上去了。期刊 manifest 262 列全滿、0 列對不上。

  **《秘技大全》與《攻略大全》建過又刪**：它們有 ISBN、自稱「攻略叢書系列」，是有期號的
  叢書不是定期刊物，tocr 只收期刊。三張封面改歸 `raw/game-books/` 並換成書的命名。
  對照組是《攻略快報》——ISSN 1022-4823、半月刊、每月 5 日與 20 日出刊，那才是期刊。

  電擊GAMES 的 Vol.26 不是打錯：2003 年 2 月號是改名後的第一本、仍沿用《電擊王》的編號，
  3 月號起才從 Vol.2 另編一段；站上那期的 notes 本來就這樣寫。腳本
  `tocr_create_new_scans.py` 留在 nostalib-toolkit，`tocr_upload_covers.py` 加了
  「`derived/` 有同名檔就優先用」。細節見
  [covers-new-scans-tocr.md](covers-new-scans-tocr.md)
