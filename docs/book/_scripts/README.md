...
`gen.mjs` 會整份重產 `../magazines/NN-slug.md`，但**依 slug 把既有那份稿的
〈沿革〉〈可寫的話題〉〈出處〉三節原樣搬過來**，所以人寫的部分不會掉。編號前綴依
創刊日重算，新增雜誌會讓後面的檔名整批位移（舊檔一併刪掉）——**位移不會動到
`../README.md`、`../front-matter/` 與 `../columns/` 裡以「NN 刊名」形式寫的交叉
引用，那些要自己對一遍**；新增的刊也要補寫上面那三節。
