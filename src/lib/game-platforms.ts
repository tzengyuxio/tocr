 ...
 * 「這幾顆是任天堂、那幾顆是索尼」。
 *
 * 明度預設 0.45：籌碼上的字只有 12px，未選取時是彩色字配白底、選取時是白字配
 * 彩色底，兩個方向都要有 4.5:1 以上——而這兩個方向算出來是同一個數字（都是白對
 * 這個色），所以只要相對亮度壓在 0.183 以下就一起過。壓在同一個明度最省事。
 * 彩度各自調整——紅色在同明度下看起來比青色重，全部給同一個值會讓某幾顆特別搶。
 *
 * **任天堂是例外，取的是它自己的紅（#E60012）**：那個紅讀者在主機與包裝上認得，
 * 借過來比自己調一個更說得出「這是任天堂」。代價是它比別族亮、也比別族豔，一排
 * 籌碼裡會先看到它——這是刻意換來的，不是沒注意到。相對亮度 0.169，仍然在
 * 0.183 以下，兩個方向都還有 4.8:1。
 *
