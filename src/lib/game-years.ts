 ...
 * 每一年有幾款遊戲被寫到，以及資料的兩端。
 *
 * **不跟著當下的篩選走**，永遠是整個站的分布（yuxio 2026-09-20 確認）。它同時是
 * 篩選器與密度圖，而密度圖要當得成地標就不能一直變形——讀者打了關鍵字之後，
 * 長條該還是那條長條，只是選取的那段換了意思。代價是「選了 PC 之後 1999 那格
 * 仍然是 764」，可以接受：那格說的是「1999 年的雜誌寫了 764 款遊戲」，本來就與
 * 平台無關。
 *
